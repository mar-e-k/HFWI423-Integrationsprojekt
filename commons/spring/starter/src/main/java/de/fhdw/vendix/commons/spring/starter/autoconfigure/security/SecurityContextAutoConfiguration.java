...
public class SecurityContextAutoConfiguration {
    ...
    @Bean
    @ConditionalOnMissingBean
    public ContextInitializationDelegator contextInitializationDelegator(AppContext appContext, ConnectionProxyService connectionProxyService, LockProxyService lockProxyService) {
        return new DefaultContextInitializationDelegator(appContext, connectionProxyService, lockProxyService);
    }

    @Bean
    @ConditionalOnMissingBean
    public LoginHandler loginHandler(AppContext appContext, LockProxyService lockProxyService) {
        return new DefaultLoginHandler(appContext, lockProxyService);
    }

    @Bean
    @ConditionalOnMissingBean
    public LogoutHandler logoutHandler(LockProxyService lockProxyService) {
        return new DefaultLogoutHandler(lockProxyService);
    }

    @Bean
    @ConditionalOnMissingBean
    public ShutdownHandler shutdownHandler(AppContext appContext, LockProxyService lockProxyService, ConnectionProxyService connectionProxyService) {
        return new DefaultShutdownHandler(appContext, lockProxyService, connectionProxyService);
    }
}