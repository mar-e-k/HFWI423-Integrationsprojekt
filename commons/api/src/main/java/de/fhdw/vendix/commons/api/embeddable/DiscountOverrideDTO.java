...
public record DiscountOverrideDTO(
    ...
    public DiscountOverrideDTO {
        if (amount == null) {
            throw new IllegalArgumentException("DiscountOverrideDTO parameter 'articleAmount' cannot be null");
        }
        if (amount.compareTo(BigDecimal.valueOf(0)) < 0 || amount.compareTo(BigDecimal.valueOf(100)) > 0) {
            throw new IllegalArgumentException("DiscountOverrideDTO parameter 'articleAmount' must be between 0 and 100");
        }
        if (reason == null) {
