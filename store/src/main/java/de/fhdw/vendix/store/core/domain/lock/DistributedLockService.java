...
public class DistributedLockService extends AbstractCrudService<DistributedLock, Long> {
    ...
    public Long deleteByLockTypeAndTargetId(LockTypeEnum lockType, Long targetId) {
        ...
        if (deletedLockAmount > 0) {
            log.atInfo().log("[DELETED] [{}] with targetType [{}] and id [{}] with lock amount [{}]", this.getClass().getSimpleName(), lockType.name(), targetId, deletedLockAmount);
        } else {
            log.atInfo().log("[DELETED] [{}] with targetType [{}] and id [{}] has no locks to delete", this.getClass().getSimpleName(), lockType.name(), targetId);
        }
        return deletedLockAmount;
    }
}